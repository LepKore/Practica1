...
Uso:
    ...
    python inventario.py --root . --incluir-ignorados
...
def recorrer_arbol(raiz: Path, respeta_gitignore: bool, incluir_ignorados: bool = False) -> tuple[list[Path], set[str]]:
    """Recorre el árbol y devuelve (candidatos, rutas_ignoradas_por_git).

    Por defecto la lista de git manda, como siempre. Con incluir_ignorados=True los
    archivos que .gitignore descarta también entran, marcados con 'ignorado_por_git': True,
    porque un CONTRIBUTING.md sin trackear documenta el proyecto igual que uno trackeado y
    la diferencia es del .gitignore, no del autor. Útil cuando el repo ignora *.md.
    """
    ignorados: set[str] = set()
    descartados: set[str] = set()
    if respeta_gitignore:
        consultas = [["--cached", "--others", "--exclude-standard"]]
        if incluir_ignorados:
            consultas.append(["--others", "--ignored", "--exclude-standard"])
        for flags in consultas:
            destino = descartados if "--ignored" in flags else ignorados
            try:
                ...
                if salida.returncode == 0:
                    destino.update(
                        l.strip().replace("\\", "/") for l in salida.stdout.splitlines() if l.strip()
                    )
            except (OSError, subprocess.SubprocessError):
                destino.clear()

    permitidos = ignorados | descartados
    ...
    for ruta in raiz.rglob("*"):
        ...
        if respeta_gitignore and permitidos and posix not in permitidos:
            continue
    ...
    return sorted(candidatos), descartados
...
def construir_inventario(raiz: Path, muestra: int, incluir_ignorados: bool = False) -> list[dict]:
    en_git = es_repo_git(raiz)
    candidatos, descartados = recorrer_arbol(raiz, respeta_gitignore=en_git,
                                             incluir_ignorados=incluir_ignorados)
    ...
    for ruta in candidatos:
        ...
        if rel in descartados:
            entrada["ignorado_por_git"] = True
...
def main() -> int:
    ...
    ap.add_argument("--incluir-ignorados", action="store_true",
                    help="Incluye también los archivos que .gitignore descarta, marcados con "
                         "ignorado_por_git. Útil en repos que ignoran *.md, donde si no un "
                         "CONTRIBUTING.md o un docs/arquitectura.md desaparecen del informe")
    args = ap.parse_args()
    ...
    inventario = construir_inventario(raiz, args.muestra, args.incluir_ignorados)
    en_git = es_repo_git(raiz)
    ...
    for a in inventario:
        ...
        if a.get("ignorado_por_git"):
            tipo += " (ignorado por git)"
        kb = a["bytes"] / 1024
